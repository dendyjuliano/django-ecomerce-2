...
class Cart:
    def __init__(self, request):
        ...
        if cart_id:
            cart = models.Cart.objects.filter(
                id=cart_id, checked_out=False).first()
            if cart is None:
    ...
    def add(self, product, unit_price, quantity=1):
        ...
        if item:
            ...
            models.Item.objects.create(
                cart=self.cart, product=product, unit_price=unit_price, quantity=quantity)
    ...
    def increment(self, product, quantity, unit_price=None):
        item = models.Item.objects.filter(
            cart=self.cart, product=product).first()
        if item:
    ...
    def decrement(self, product, quantity, unit_price=None):
        item = models.Item.objects.filter(
            cart=self.cart, product=product).first()
        if item:
            if item.quantity == 1:
